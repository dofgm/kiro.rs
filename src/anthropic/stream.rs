            ...
            events.push(SseEvent::new(
                ...
                json!({
                    ...
                    "usage": {
                        ...
                        "credits_used": if credits_used.is_finite() { credits_used.max(0.0) } else { 0.0 }
                    }
                }),
            ));
        }
...
pub struct StreamContext {
    ...
    /// 是否使用 contextUsageEvent 覆写 input_tokens（/cc/v1/messages 需要；/v1/messages 用估算值保证稳定）
    pub use_context_input_tokens: bool,
    /// 输出 tokens 累计
    pub output_tokens: i32,
    /// prompt cache: 本次写入 cache 的输入 tokens（模拟值）
    pub cache_creation_input_tokens: i32,
    /// prompt cache: 本次从 cache 读取的输入 tokens（模拟值）
    pub cache_read_input_tokens: i32,
    /// 上游 meteringEvent 累积的 credits（用于对照/展示）
...
impl StreamContext {
    ...
    pub fn generate_final_events(&mut self) -> Vec<SseEvent> {
        ...
        // 对外返回的 input_tokens 口径：总输入扣除缓存读/写
        let final_input_tokens = self.response_input_tokens();
    ...
    /// 获取最终 usage（优先使用 contextUsageEvent 计算出的 input tokens）
    pub fn final_usage_tokens(&self) -> (i32, i32) {
        (self.total_input_tokens(), self.output_tokens.max(0))
    }

    /// 当前请求的总输入 tokens（可能包含可缓存部分）
    ...
    /// 对外返回的输入 tokens（不包含缓存读/写）
    pub fn response_input_tokens(&self) -> i32 {
        let total = self.total_input_tokens() as i64;
        let cache_creation = (self.cache_creation_input_tokens.max(0)) as i64;
        let cache_read = (self.cache_read_input_tokens.max(0)) as i64;
    ...
    pub fn set_extra_usage(&mut self, cache_creation: i32, cache_read: i32, credits_used: f64) {
        self.cache_creation_input_tokens = cache_creation.max(0);
        self.cache_read_input_tokens = cache_read.max(0);
        self.credits_used = if credits_used.is_finite() {
...
impl BufferedStreamContext {
    ...
    pub fn new(
        ...
        let inner =
            StreamContext::new_with_thinking(model, estimated_input_tokens, thinking_enabled, true);
        Self {
            inner,
    ...
    pub fn finish_and_get_all_events(&mut self) -> Vec<SseEvent> {
        ...
        // 获取对外返回的 input_tokens（不包含缓存读/写）
        let final_input_tokens = self.inner.response_input_tokens();
    ...
    /// 获取最终 usage（优先使用 contextUsageEvent 计算出的 input tokens）
    pub fn final_usage_tokens(&self) -> (i32, i32) {
        self.inner.final_usage_tokens()
    }

    /// 获取创建时的估算 input_tokens（不会被 contextUsageEvent 覆写）
    pub fn estimated_input_tokens(&self) -> i32 {
        self.estimated_input_tokens.max(0)
    }
}
...
mod tests {
    ...
    fn test_text_delta_after_tool_use_restarts_text_block() {
        let mut ctx = StreamContext::new_with_thinking("test-model", 1, false, false);
    ...
    fn test_tool_use_flushes_pending_thinking_buffer_text_before_tool_block() {
        ...
        let mut ctx = StreamContext::new_with_thinking("test-model", 1, true, false);
