...
///
/// 条件：tools 有且只有一个，且该工具可识别为 WebSearch
...
fn is_web_search_tool(tool: &super::types::Tool) -> bool {
    if tool
        .tool_type
        .as_ref()
        .is_some_and(|t| t.to_ascii_lowercase().starts_with("web_search"))
    {
        return true;
    }

    let normalized_name: String = tool
        .name
        .chars()
...
pub fn extract_search_query(req: &MessagesRequest) -> Option<String> {
    ...
    let text = match &target_message.content {
        ...
        serde_json::Value::Array(arr) => {
            ...
            text_block.get("text")?.as_str()?.trim().to_string()
        }
        _ => return None,
    };

    // 去除前缀 "Perform a web search for the query: "（兼容大小写，尾部空格可选）
    // 注意：必须避免在非 ASCII 文本上按字节索引切片，否则会触发 UTF-8 边界 panic
...
mod tests {
    ...
    #[test]
    fn test_has_web_search_tool_claude_sdk_name() {
        use crate::anthropic::types::{Message, Tool};
    ...
    fn test_extract_search_query_uses_last_user_message() {
        ...
        let req = MessagesRequest {
            ...
            messages: vec![
                ...
                Message {
                    ...
                    content: serde_json::json!("intermediate"),
                },
                Message {
                    role: "user".to_string(),
                    content: serde_json::json!([{
                        "type": "text",
                        "text": "Perform a web search for the query: newest mcp support in social platforms"
                    }]),
                },
            ],
            stream: true,
        ...
        assert_eq!(
            query,
            Some("newest mcp support in social platforms".to_string())
        );
    }

    #[test]
    fn test_extract_search_query_chinese_without_prefix() {
        use crate::anthropic::types::Message;
        ...
        let req = MessagesRequest {
            ...
            messages: vec![Message {
                role: "user".to_string(),
                content: serde_json::json!("查看各个社媒平台他们是否已经提供了对应的 mcp"),
            }],
            stream: true,
            system: None,
            tools: None,
        ...
        assert_eq!(
            query,
            Some("查看各个社媒平台他们是否已经提供了对应的 mcp".to_string())
        );
    }
