...
pub async fn post_messages(State(state): State<AppState>, body: BodyBytes) -> Response {
    ...
    if provider.token_manager().get_strip_billing_header() {
...
async fn handle_stream_request(
    ...
    // 创建流处理上下文
    let mut ctx = StreamContext::new_with_thinking(model, input_tokens, thinking_enabled, false);
                ...
                chunk_result = body_stream.next() => {
                    match chunk_result {
                                ...
                                match result {
                                    Ok(frame) => {
                                        if let Ok(event) = Event::from_frame(frame) {
                                            if let Event::Metering(metering) = &event {
                                                credits_used += metering.usage.max(0.0);
                                            }
                        ...
                        Some(Err(e)) => {
                            ...
                            token_manager.report_credits(credential_id, credits_used);
                            let (_, final_output_tokens) = ctx.final_usage_tokens();
                            // KV cache 模拟使用“本地估算的可见 prompt tokens”，保证统计稳定可复现。
...
async fn handle_non_stream_request(
    ...
    let mut request_credits_used = 0.0_f64;
                ...
                if let Ok(event) = Event::from_frame(frame) {
                    match event {
                        ...
                        Event::Metering(metering) => {
                            request_credits_used += metering.usage.max(0.0);
                        }
                        Event::Error {
                            error_code,
                            error_message,
    ...
    provider
        .token_manager()
        .report_credits(credential_id, request_credits_used);
...
mod tests {
    ...
    fn test_parse_json_with_control_char_tolerance_count_tokens_with_newline() {
        let body = format!(
            r#"{{"model":"claude-haiku-4-5-20251001","messages":[{{"role":"user","content":"hello{}world"}}],"system":null,"tools":null}}"#,
            '\n'
        );
