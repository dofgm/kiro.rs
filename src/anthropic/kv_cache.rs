...
fn cache_dir(token_manager: &MultiTokenManager) -> PathBuf {
    token_manager
        .cache_dir()
        .unwrap_or_else(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")))
}

fn records_file_path(token_manager: &MultiTokenManager) -> PathBuf {
    cache_dir(token_manager).join(KV_RECORDS_FILE)
}

fn namespace_key(endpoint: &str, model: &str, credential_id: u64) -> String {
    format!("{}|{}|{}", endpoint, credential_id, model)
}
...
fn record_impl(
    ...
    let dir = cache_dir(token_manager);
    fs::create_dir_all(&dir)?;

    let records_path = records_file_path(token_manager);
    ...
    let ns_key = namespace_key(input.endpoint, &input.model, input.credential_id);
...
pub fn record_simulated_kv_cache(
    ...
    let result = if tokio::runtime::Handle::try_current().is_ok() {
        tokio::task::block_in_place(|| record_impl(token_manager, input))
    } else {
        record_impl(token_manager, input)
    };
