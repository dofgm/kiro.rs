...
pub fn count_tokens(text: &str) -> u64 {
    // 空字符串按 0 计；非空至少 1 token，避免短文本被截断成 0。
    if text.is_empty() {
        return 0;
    }
...
fn count_all_tokens_local(
    ...
    fn count_json_value_tokens(value: &serde_json::Value) -> u64 {
        match value {
            ...
            serde_json::Value::Bool(b) => count_tokens(if *b { "true" } else { "false" }),
            serde_json::Value::Number(n) => count_tokens(&n.to_string()),
            serde_json::Value::String(s) => count_tokens(s),
            serde_json::Value::Array(arr) => arr.iter().map(count_json_value_tokens).sum(),
            serde_json::Value::Object(obj) => {
                // For unknown shapes, serialize to keep the estimate monotonic.
    ...
    fn count_content_block_tokens(block: &serde_json::Value) -> u64 {
        ...
        // Fallback: if we still counted nothing but the block is non-empty,
        // charge at least 1 token to keep growth monotonic for structural blocks.
        if total == 0 {
            total = 1;
        }
    ...
    // 用户/助手消息（按 content blocks 估算，尽量覆盖 tool_use/tool_result）
    for msg in &messages {
        match &msg.content {
            ...
            other => {
                total += count_json_value_tokens(other);
            }
        }
    }
