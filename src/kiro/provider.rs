...
impl KiroProvider {
    ...
    /// 获取 token_manager 的 Arc（用于跨异步边界共享）
    ...
    /// 从请求体中提取路由键（用于会话粘性路由）
    ///
    /// 优先使用 conversationState.conversationId
    ...
    fn build_headers(&self, ctx: &CallContext) -> anyhow::Result<HeaderMap> {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(
                "aws-sdk-rust/1.3.9 ua/2.1 api/codewhispererstreaming/0.1.11582 os/windows \
                 lang/rust/1.87.0 md/appVersion-1.19.4 app/AmazonQ-For-CLI",
            ),
        );
        headers.insert(
            "x-amz-user-agent",
            HeaderValue::from_static(
                "aws-sdk-rust/1.3.9 ua/2.1 api/codewhispererstreaming/0.1.11582 os/windows \
                 lang/rust/1.87.0 m/F app/AmazonQ-For-CLI",
            ),
        );
        headers.insert(
    ...
    ///
    /// # Returns
    /// 返回原始的 HTTP Response 和本次使用的凭据 ID，不做解析
    ...
    ///
    /// # Returns
    /// 返回原始的 HTTP Response 和本次使用的凭据 ID，调用方负责处理流式数据
    ...
    async fn call_mcp_with_retry(&self, request_body: &str) -> anyhow::Result<reqwest::Response> {
        ...
        for attempt in 0..max_retries {
            ...
            let ctx = match self
                .token_manager
                .acquire_context(None, route_key.as_deref())
                .await
            {
                Ok(c) => c,
                Err(e) => {
    ...
    async fn call_api_with_retry(
        ...
        for attempt in 0..max_retries {
            ...
            let ctx = match self
                .token_manager
                .acquire_context(model.as_deref(), route_key.as_deref())
                .await
            {
                Ok(c) => c,
                Err(e) => {
...
mod tests {
    ...
    fn test_build_headers() {
        ...
        assert_eq!(
            headers.get("x-amz-user-agent").unwrap(),
            "aws-sdk-rust/1.3.9 ua/2.1 api/codewhispererstreaming/0.1.11582 os/windows \
             lang/rust/1.87.0 m/F app/AmazonQ-For-CLI"
        );
        assert_eq!(headers.get("x-amzn-codewhisperer-optout").unwrap(), "false");
    ...
    #[test]
    fn test_extract_route_key_from_request_with_conversation_id() {
        let request_body = r#"{
            "conversationState": {
        ...
        let route_key = KiroProvider::extract_route_key_from_request(request_body);
        assert_eq!(route_key, Some("conversation:session-abc-123".to_string()));
    }

    #[test]
    fn test_extract_route_key_from_request_without_conversation_id() {
        let request_body = r#"{
            "conversationState": {
                "currentMessage": {
                    "userInputMessage": {
