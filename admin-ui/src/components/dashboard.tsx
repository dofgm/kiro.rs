...
import { RefreshCw, LogOut, Moon, Sun, Server, Plus, Upload, FileUp, Trash2, RotateCcw, CheckCircle2, ClipboardList, Settings } from 'lucide-react'
...
const LOAD_BALANCING_MODE_SEQUENCE: LoadBalancingMode[] = [
  'priority',
  'balanced',
  'weighted_round_robin',
]

const LOAD_BALANCING_MODE_LABELS: Record<LoadBalancingMode, string> = {
  priority: '优先级模式',
  balanced: '均衡负载',
  weighted_round_robin: '加权轮询',
}

function getNextLoadBalancingMode(mode: LoadBalancingMode): LoadBalancingMode {
  const currentIndex = LOAD_BALANCING_MODE_SEQUENCE.indexOf(mode)
  const nextIndex = (currentIndex + 1) % LOAD_BALANCING_MODE_SEQUENCE.length
  return LOAD_BALANCING_MODE_SEQUENCE[nextIndex]
}
...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  // 按订阅类型筛选
  const filteredCredentials = (data?.credentials || []).filter(credential => {
    if (subscriptionFilter === 'all') return true
    if (subscriptionFilter === 'unknown') return !credential.subscriptionTitle
    return credential.subscriptionTitle?.toUpperCase().includes(subscriptionFilter.toUpperCase())
  })
  ...
  // 当凭据列表变化或筛选条件变化时重置到第一页
  useEffect(() => {
    setCurrentPage(1)
  }, [data?.credentials.length, subscriptionFilter])
  ...
  const handleToggleLoadBalancing = () => {
    ...
    const newMode = getNextLoadBalancingMode(currentMode)

    setLoadBalancingMode(newMode, {
      onSuccess: () => {
        toast.success(`已切换到${LOAD_BALANCING_MODE_LABELS[newMode]}`)
      },
      onError: (error) => {
        ...
        <div className="container flex h-14 items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              title="切换负载均衡模式（优先级/均衡/加权轮询）"
            >
              {isLoadingMode
                ? '加载中...'
                : LOAD_BALANCING_MODE_LABELS[loadBalancingData?.mode || 'priority']}
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={() => setRequestDetailsDialogOpen(true)}
            >
      ...
      <main className="container mx-auto px-4 md:px-8 py-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {['all', 'FREE', 'PRO+', 'PRO', 'POWER', 'unknown'].map((filter) => {
              const label = filter === 'all' ? '全部' : filter === 'unknown' ? '未知' : filter
              const count = filter === 'all'
                ? data?.credentials.length || 0
                : filter === 'unknown'
                  ? (data?.credentials || []).filter(c => !c.subscriptionTitle).length
                  : (data?.credentials || []).filter(c => c.subscriptionTitle?.toUpperCase().includes(filter)).length
              return (
                <Button
                  key={filter}
                  size="sm"
